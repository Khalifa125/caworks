...
const STATS = [
  { num: '8', label: 'Years of London Excellence' },
  { num: '48h', label: 'Quote Turnaround' },
  { num: '300°', label: 'Heat Resistance' },
]

export default function Hero() {
  return (
    <section className="relative min-h-svh lg:min-h-screen flex flex-col justify-end px-6 md:px-8 lg:px-14 pb-16 md:pb-20 overflow-hidden">
      {/* Background */}
      ...
      <motion.div {...fadeUp(0)} className="flex items-center gap-4 mb-7">
        ...
        <span className="flex-1 h-[1px] max-w-[80px]" style={{ background: 'var(--color-rule)' }} />
      </motion.div>

      {/* Headline */}
      <motion.h1
        {...fadeUp(0.15)}
        className="font-display text-[clamp(52px,7.5vw,108px)] font-light leading-[1.02] -tracking-[0.01em] mb-8 max-w-[760px]"
      >
      ...
      <motion.p
        {...fadeUp(0.3)}
        className="max-w-[520px] text-sm text-ivory-dim leading-[1.9] font-light mb-[52px]"
      >
      ...
      <motion.div {...fadeUp(0.45)} className="flex items-center gap-8 flex-wrap">
        ...
        <a
          href="#products"
          className="text-xs tracking-[0.22em] uppercase font-medium text-ivory-dim no-underline flex items-center gap-2.5 transition-colors duration-300 hover:text-gold-light group"
        >
          Explore Our Work
      ...
      >
        {STATS.map((s) => (
          <div key={s.num} className="text-left lg:text-right">
            <div className="font-display text-[2.6rem] font-light text-gold-light leading-none -tracking-[0.02em]">
              {s.num}
            </div>
            <div className="text-[0.58rem] tracking-[0.2em] uppercase text-ivory-dim mt-1">
              {s.label}
            </div>
          </div>
        ))}
